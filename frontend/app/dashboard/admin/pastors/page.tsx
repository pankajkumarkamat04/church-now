...
export default function AdminPastorsPage() {
  ...
  useEffect(() => {
    ...
    loadTerms().catch((e) => setErr(e instanceof Error ? e.message : 'Failed to load pastor terms'));
  }, [token, user]);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
        <h1 className="text-xl font-semibold text-neutral-900">Pastor term management</h1>
      ...
      <div className="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm">
            ...
            {rows.map((row) => (
              <tr key={row._id} className="border-b border-neutral-100 last:border-0">
                ...
                <td className="px-4 py-3 text-right">
                  <button
                    type="button"
                    disabled={busy || row.status === 'TRANSFER_REQUIRED' || row.termNumber >= 2}
        ...
        {rows.length === 0 ? <p className="px-4 py-8 text-center text-sm text-neutral-500">No pastor terms yet.</p> : null}
      </div>
      <PastorAssignModal
        open={assignOpen}
