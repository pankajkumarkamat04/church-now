...
export default function SuperadminPastorsPage() {
  ...
  useEffect(() => {
    ...
    load().catch((e) => setErr(e instanceof Error ? e.message : 'Failed to load pastor records'));
  }, [token, user]);
  ...
  return (
    <div className="max-w-6xl">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-violet-700">Pastor records</p>
          <h1 className="mt-1 text-2xl font-semibold tracking-tight text-neutral-900 sm:text-3xl">
            Church spiritual leaders
          </h1>
          <p className="mt-1 text-sm text-neutral-600">Manage assignments, renewals and transfers across all churches.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm">
            ...
            {filtered.map((row) => (
              <tr key={row._id} className="border-b border-neutral-100 last:border-0">
                <td className="px-4 py-3">
                  {typeof row.church === 'object' && row.church ? row.church.name || '—' : '—'}
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex justify-end gap-2">
                    <button
                      type="button"
                      disabled={busyTermId === row._id || row.status === 'TRANSFER_REQUIRED' || row.termNumber >= 2}
        ...
        {filtered.length === 0 ? (
          <p className="px-4 py-8 text-center text-sm text-neutral-500">No pastor records found.</p>
        ) : null}
      </div>
    </div>
  );
