...
export default function EditServicePage() {
  ...
  const loadBase = useCallback(async () => {
    ...
    let foundService:
      | (NonNullable<ServiceCouncil['services']>[number] & { _councilId: string })
      | null = null;
    for (const c of serviceCouncils) {
      const s = (c.services || []).find((x) => String(x._id) === serviceId);
      if (s) {
        foundService = { ...s, _councilId: c._id };
        foundCouncilId = c._id;
        break;
      }
    }
    if (!foundService) {
    ...
    setCouncilId(foundCouncilId);
    setName(foundService.name || '');
    setContactName(foundService.contactName || '');
    setContactPhone(foundService.contactPhone || '');
    setContactEmail(foundService.contactEmail || '');
    setHeadMemberId(
      typeof foundService.head === 'object' ? foundService.head?._id || '' : String(foundService.head || '')
    );
  }, [token, requestedCouncilId, serviceId]);
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      await apiFetch(
        `/api/superadmin/main-churches/${mainChurchId}/service-councils/${councilId}/services/${serviceId}`,
        {
          method: 'PUT',
          token,
          body: JSON.stringify({
    ...
    <div className="w-full min-w-0 max-w-3xl">
      <div className="mb-6 flex items-center justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-600">Update service details and re-assign its head.</p>
        </div>
        <Link
      ...
      <form onSubmit={onSave} className="space-y-4 rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
        <div>
          <label className="mb-1 block text-xs font-medium text-neutral-600">Service council</label>
          <select value={councilId} onChange={(e) => setCouncilId(e.target.value)} className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm" required>
