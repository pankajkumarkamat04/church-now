...
export default function SuperadminFinanceExpensesPage() {
  ...
  const load = useCallback(async () => {
    if (!token) return;
    const q = filterChurch ? `?churchId=${encodeURIComponent(filterChurch)}` : '';
  ...
  useEffect(() => {
    if (churches.length && !churchId) setChurchId(churches[0]._id);
  }, [churches, churchId]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const body = {
        churchId: editing ? undefined : churchId,
        title: title.trim(),
      ...
      if (editing) {
        await apiFetch(`/api/superadmin/expenses/${editing._id}`, { method: 'PUT', token, body: JSON.stringify(body) });
      } else {
        if (!churchId) {
          setErr('Church is required');
          return;
        }
        await apiFetch('/api/superadmin/expenses', { method: 'POST', token, body: JSON.stringify({ ...body, churchId }) });
      }
      resetForm();
      await load();
    } catch (error) {
  ...
  return (
    <div className="w-full min-w-0 max-w-6xl">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-neutral-600">Add or review expenses for any church.</p>
      </div>
      {err ? <p className="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800">{err}</p> : null}

      <div className="mb-4">
        <label className="mb-1 block text-xs font-medium text-neutral-600">Filter list by church</label>
        <select
          value={filterChurch}
          onChange={(e) => setFilterChurch(e.target.value)}
          className="max-w-md rounded-lg border border-neutral-300 px-3 py-2 text-sm"
        >
            ...
            {rows.map((r) => (
              <tr key={r._id} className="border-t border-neutral-100">
                ...
                <td className="px-4 py-2 text-right">
                  <button type="button" onClick={() => startEdit(r)} className="mr-2 inline-flex items-center text-violet-700 hover:underline">
