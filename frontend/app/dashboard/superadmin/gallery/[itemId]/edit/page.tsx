...
export default function SuperadminEditGalleryItemPage() {
  ...
  useEffect(() => {
    async function loadData() {
      if (!token || !itemId || !originChurchId) return;
      setLoadErr(null);
      setFetching(true);
      try {
        const item = await apiFetch<SuperadminGalleryRecord>(
          `/api/superadmin/churches/${originChurchId}/gallery/${itemId}`,
          { token }
        );
        ...
        setShowOnHome(Boolean(item.showOnHome));
      } catch (error) {
        setLoadErr(error instanceof Error ? error.message : 'Failed to load item');
      } finally {
        setFetching(false);
      }
    }
    loadData();
  }, [token, itemId, originChurchId]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (churchId === originChurchId) {
        await apiFetch(`/api/superadmin/churches/${originChurchId}/gallery/${itemId}`, {
          method: 'PUT',
          token,
    ...
    return (
      <div className="mx-auto max-w-xl">
        <p className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800">
          Church id is missing. Please open edit from the gallery list.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6">
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight text-neutral-900 sm:text-3xl">Edit gallery item</h1>
      </div>
