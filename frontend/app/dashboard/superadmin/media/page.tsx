...
export default function SuperadminMediaPage() {
  ...
  async function load() {
    ...
    setFetching(true);
    try {
      const data = await apiFetch<MediaItem[]>('/api/superadmin/media?limit=300', { token });
      setItems(data);
    } catch (error) {
    ...
    <div className="w-full min-w-0 max-w-6xl">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-600">Upload and manage image assets for events, gallery, and frontend sections.</p>
        </div>
        <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-violet-500">
              ...
              <div className="p-2">
                ...
                <div className="mt-2 flex gap-2">
                  <button
                    ...
                    className="inline-flex flex-1 items-center justify-center gap-1 rounded border border-violet-300 bg-violet-50 px-2 py-1 text-xs font-medium text-violet-900 hover:bg-violet-100"
                  >
